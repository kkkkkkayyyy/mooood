        ...
        <div className='flex flex-col gap-2.5'>
          ...
          <div className='py-2 text-center'>
            <h1 className='font-chewy' style={{ fontSize: 32, color: '#272724', lineHeight: 1.2 }}>
              Hagamos un check-in rapido, Sofia
            </h1>
          </div>
        </div>
      </div>
      <div className='flex-1 overflow-y-auto scroll-hide px-5 pb-8'>
        <p className='font-quicksand font-bold text-center opacity-70 mb-10 mt-4' style={{ fontSize: 18, color: '#272724' }}>
        ...
        <div className='grid grid-cols-2 gap-x-4 gap-y-8'>
          {emotions.map(({ key, label, img, nextScreen }) => (
            <button key={key} onClick={() => onNavigate(nextScreen)} className='flex flex-col items-center gap-5 card-press'>
              ...
              <span className='font-quicksand' style={{ fontSize: 16, color: '#272724' }}>{label}</span>
            </button>
          ))}
        </div>
      </div>
